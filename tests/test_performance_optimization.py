...
class TestOptimizedDoclingReader:
    ...
    def test_optimized_reader_initialization(self):
        """Test DoclingJsonReader with performance options."""
    ...
    def test_json_parser_selection(self):
        ...
        reader = DoclingJsonReader(use_fast_json=True)
        parser = reader._json_parser
        assert parser is not None

        # Test with fast JSON disabled
        reader_no_fast = DoclingJsonReader(use_fast_json=False)
    ...
    def test_performance_monitoring_context(self):
        """Test performance monitoring with DoclingJsonReader."""
        reader = DoclingJsonReader()

        # Performance monitoring is now integrated into operations
        # No need for explicit context manager - it's handled internally
        assert reader.performance_config is not None
        ...
        try:
            with tempfile.NamedTemporaryFile(mode='w', suffix='.json', delete=False) as f:
                json.dump({"schema_name": "DoclingDocument", "version": "1.0.0"}, f)
                temp_file = Path(f.name)

            # This operation should be internally monitored
            try:
                reader.load_data(temp_file)
            except Exception:
                pass  # Expected to fail with minimal schema
        finally:
            if temp_file:
                temp_file.unlink()
        assert True
    ...
    def test_preload_documents(self):
        ...
        try:
            ...
            # Load documents (will be cached)
            results = {}
            for file_path in test_files:
                try:
                    results[file_path] = reader.load_data(file_path)
...
class TestOptimizedLexicalSerializer:
    ...
    def test_optimized_serializer_initialization(self):
        """Test LexicalDocSerializer with performance options."""
        # Create test document
    ...
    def test_optimized_params_validation(self):
        """Test LexicalParams validation with performance options."""
        # Valid params
        params = LexicalParams(batch_size=100, max_workers=2)
        assert params.batch_size == 100
        assert params.max_workers == 2

        # Test default values
        default_params = LexicalParams()
        assert default_params.enable_streaming is None  # Auto-detect
    ...
    def test_json_encoder_selection(self):
        ...
        # Test with fast JSON enabled
        params_fast = LexicalParams(use_fast_json=True)
        serializer_fast = LexicalDocSerializer(doc=doc, params=params_fast)
        encoder_fast = serializer_fast._json_encoder
        assert encoder_fast is not None

        # Test with fast JSON disabled
        params_standard = LexicalParams(use_fast_json=False)
        serializer_standard = LexicalDocSerializer(doc=doc, params=params_standard)
    ...
    def test_optimized_text_processing(self):
        ...
        params = LexicalParams(optimize_text_formatting=True)
        serializer = LexicalDocSerializer(doc=doc, params=params)

        text_item = doc.texts[0]

        # Test fast link processing
        nodes = serializer._process_text_with_links_optimized("Visit https://example.com for more info", text_item)
        assert len(nodes) >= 2  # Should have text and link nodes

        # Test fast formatting detection
        format_types = serializer._detect_text_formatting_optimized("IMPORTANT MESSAGE", text_item)
    ...
    def test_serialize_optimization_modes(self):
        ...
        params_streaming = LexicalParams(
            enable_streaming=True,
            batch_size=1
        )
        serializer_streaming = LexicalDocSerializer(doc=doc, params=params_streaming)
