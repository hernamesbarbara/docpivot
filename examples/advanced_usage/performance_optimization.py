...
def best_practices_summary():
    ...
    print(f"   1. Document caching (highest impact)")
    print(f"   2. Parallel processing")
    print(f"   3. Compact configuration")
    print(f"   4. Memory management")
    print(f"   5. Streaming/chunking")
    
    print(f"\n⚠️  Common performance pitfalls:")
    print(f"   • Loading the same document multiple times")
    print(f"   • Using verbose serialization options unnecessarily")
    print(f"   • Not cleaning up large objects")
    print(f"   • Processing large batches sequentially")
    print(f"   • Ignoring memory profiling in production")
...
def main():
    ...
    try:
        ...
        print(f"   • PerformanceTimer (custom timing context manager)")
        print(f"   • memory_profiler (memory usage analysis)")
        print(f"   • concurrent.futures (parallel processing)")
        print(f"   • garbage collection (memory management)")
        
        print(f"\n📁 Generated files:")
        print(f"   • Check current directory for any output files")
