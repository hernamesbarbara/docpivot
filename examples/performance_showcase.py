...
def demonstrate_performance_monitoring(sample_docs: List[Path]):
    ...
    result = monitor.profile_reader(DoclingJsonReader, sample_docs[1])  # Medium document
    ...
    print("\nProfiling OptimizedDoclingJsonReader...")
    result_opt = monitor.profile_reader(OptimizedDoclingJsonReader, sample_docs[1])
    ...
    print("\nProfiling OptimizedLexicalDocSerializer...")
    opt_params = OptimizedLexicalParams(
        enable_streaming=True,
        use_fast_json=True,
        optimize_text_formatting=True,
        memory_efficient_mode=True
    )
    result_opt = monitor.profile_serializer(OptimizedLexicalDocSerializer, doc, params=opt_params)
...
def demonstrate_edge_case_handling(sample_docs: List[Path]):
    ...
    # Create malformed JSON document
    malformed_json = """{
        "schema_name": "DoclingDocument",
        "version": "2.0.0",
        "name": "malformed_doc",
...
def demonstrate_optimized_readers(sample_docs: List[Path]):
    ...
    cached_reader = OptimizedDoclingJsonReader(
        enable_caching=True,
        use_fast_json=True
    )
    ...
    doc1 = cached_reader.load_data(sample_docs[1])
    first_load_time = (time.time() - start_time) * 1000
    
    # Second load (warm cache)
    start_time = time.time()
    doc2 = cached_reader.load_data(sample_docs[1])
    ...
    reader_with_progress = OptimizedDoclingJsonReader(
        progress_callback=progress_callback
    )
    
    print("Loading document with progress tracking...")
    doc = reader_with_progress.load_data(sample_docs[2])  # Large document
    ...
    batch_reader = OptimizedDoclingJsonReader(enable_caching=True)
...
def demonstrate_optimized_serializers(sample_docs: List[Path]):
    ...
    doc = docs[1]  # Medium document
    
    # Standard mode
    standard_params = OptimizedLexicalParams(
    ...
    start_time = time.time()
    standard_serializer = OptimizedLexicalDocSerializer(doc=doc, params=standard_params)
    ...
    # Optimized mode
    optimized_params = OptimizedLexicalParams(
        enable_streaming=True,
        use_fast_json=True,
        optimize_text_formatting=True,
        memory_efficient_mode=True,
    ...
    start_time = time.time()
    optimized_serializer = OptimizedLexicalDocSerializer(doc=doc, params=optimized_params)
    ...
    if len(docs) >= 2:
        parallel_params = OptimizedLexicalParams(
        ...
        for doc in docs[:2]:  # Process first 2 docs
            serializer = OptimizedLexicalDocSerializer(doc=doc, params=parallel_params)
            result = serializer.serialize()
    ...
    # Test memory-efficient mode with large document
    if len(docs) >= 3:
        memory_params = OptimizedLexicalParams(
            memory_efficient_mode=True,
        ...
        doc_large = docs[2]  # Large document
        
        start_time = time.time()
        memory_serializer = OptimizedLexicalDocSerializer(doc=doc_large, params=memory_params)
...
def run_comprehensive_benchmark(sample_docs: List[Path]):
    ...
    try:
        ...
        opt_reader_results = reader_benchmark.benchmark_file_sizes(OptimizedDoclingJsonReader)
        ...
        opt_ser_results = serializer_benchmark.benchmark_serialization_options(OptimizedLexicalDocSerializer, doc)
