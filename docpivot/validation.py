...
class DocumentValidator:
    ...
    REQUIRED_DOCLING_FIELDS = {
        "schema_name", "version", "name", "origin",
        "furniture", "body", "groups", "texts", 
        "pictures", "tables", "key_value_items", "pages"
    }
    ...
    def validate_docling_document(
        ...
        """
        logger.debug(f"Validating DoclingDocument structure{f' for {file_path}' if file_path else ''}")
        ...
        if isinstance(doc_data, DoclingDocument):
            ...
            raise ValidationError(
                ...
                context={"file_path": file_path, "actual_type": type(doc_data).__name__}
            )
    ...
    def _validate_docling_schema(self, doc_dict: Dict[str, Any], file_path: Optional[str]) -> None:
    ...
    def _validate_docling_content(self, doc_dict: Dict[str, Any], file_path: Optional[str]) -> None:
...
class LexicalValidator:
    ...
    VALID_NODE_TYPES = {
        "root", "text", "heading", "paragraph", "table", "tablerow", 
        "tablecell", "list", "listitem", "link", "image"
    }
    ...
    def validate_lexical_json(
        ...
        if not isinstance(json_data, dict):
            raise ValidationError(
                ...
                context={"file_path": file_path, "actual_type": type(json_data).__name__}
            )
    ...
    def _validate_lexical_root_node(self, root_node: Any, file_path: Optional[str]) -> None:
    ...
    def _validate_lexical_nodes(self, nodes: List[Any], file_path: Optional[str]) -> None:
    ...
    def validate_serializer_params(
        ...
        if not isinstance(params, dict):
            raise ConfigurationError(
                ...
                context={"serializer_type": serializer_type, "actual_type": type(params).__name__}
            )
        ...
        if allowed_extensions is not None:
            ...
            if file_extension not in allowed_extensions:
                raise FileAccessError(
                    ...
                    str(file_path),
                    "check_extension",
                    context={"detected_extension": file_extension, "allowed_extensions": list(allowed_extensions)}
                )
...
class JsonValidator:
    ...
    def validate_json_content(
        ...
        try:
            ...
            logger.debug(f"JSON content validation completed{f' for {file_path}' if file_path else ''}")
            return json_data
