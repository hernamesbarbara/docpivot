...
class PluginLoadError(Exception):
    ...
    def __init__(
        self, plugin_path: str, message: str, original_error: Optional[Exception] = None
    ):
...
class PluginManager:
    ...
    def register_plugin(self, plugin: FormatPlugin) -> None:
        ...
        if not isinstance(plugin, FormatPlugin):
            raise TypeError(
                f"Plugin must be a FormatPlugin instance, got {type(plugin)}"
            )
        ...
        try:
            ...
            raise PluginLoadError(
                plugin.name, f"Failed to initialize plugin '{plugin.name}': {e}", e
            )
    ...
    def load_plugin_from_module(self, module_name: str) -> None:
        ...
        try:
            ...
            raise PluginLoadError(
                module_name, f"Failed to import module '{module_name}': {e}", e
            )
    ...
    def load_plugin_from_file(self, file_path: Union[str, Path]) -> None:
        ...
        if not path.exists():
            raise PluginLoadError(str(file_path), f"Plugin file not found: {file_path}")
        ...
        try:
            ...
            if spec is None or spec.loader is None:
                raise PluginLoadError(
                    str(file_path), f"Failed to create module spec for {file_path}"
                )
            ...
            raise PluginLoadError(
                str(file_path), f"Failed to load plugin from file '{file_path}': {e}", e
            )
