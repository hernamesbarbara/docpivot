...
class OptimizedDoclingJsonReader(BaseReader):
    ...
    def __init__(
        ...
        **kwargs: Any,
        ...
        logger.debug(
            f"OptimizedDoclingJsonReader initialized with parser: {self._json_parser.__name__}"
        )
    ...
    def load_data(self, file_path: Union[str, Path], **kwargs: Any) -> DoclingDocument:
        ...
        try:
            ...
            should_stream = self.use_streaming is True or (
            ...
            perf_logger.log_file_processing(
                file_path_str, "optimized_load", duration, file_size
            )
            logger.info(
            ...
            duration = (time.time() - start_time) * 1000
            logger.error(
                f"Failed to load document from {file_path_str} after {duration:.2f}ms"
            )
            raise
    ...
    def _parse_json(self, content: str) -> Dict[str, Any]:
        ...
        try:
            ...
            if self._json_parser != json:
                logger.warning(
                    f"Fast JSON parser failed, falling back to standard: {e}"
                )
                try:
            ...
            raise DocPivotValidationError(
                ...
                error_code="JSON_PARSE_ERROR",
                context={
                    "parser": self._json_parser.__name__,
    ...
    def _validate_and_create_document(
        ...
        try:
            ...
            raise SchemaValidationError(
                ...
                context={"file_path": file_path, "validation_errors": error_details},
                cause=e,
            ) from e
    ...
    def detect_format(self, file_path: Union[str, Path]) -> bool:
        ...
        try:
            ...
            logger.warning(
                f"Error during optimized format detection for {file_path}: {e}"
            )
            return False
    ...
    def preload_documents(
    ...
    ) -> Dict[str, Union[DoclingDocument, Exception]]:
        ...
        logger.info(
            f"Preloaded {len([r for r in results.values() if isinstance(r, DoclingDocument)])} of {total_files} documents"
        )
        return results
        ...
        try:
            ...
            perf_logger.log_operation_time(
                ...
                duration_ms,
                {
                    ...
                    "parser": (
                        self._json_parser.__name__
                        if hasattr(self._json_parser, "__name__")
                        else str(type(self._json_parser))
                    ),
