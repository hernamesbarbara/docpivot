...
class PerformanceMonitor:
    ...
    def profile_reader(
        self, reader_cls, file_path: Union[str, Path], **kwargs
    ) -> Dict[str, Any]:
        ...
        with self._profile_context(operation_name) as profiler:
            try:
                ...
                logger.info(
                    f"Reader profiling complete: {duration_ms:.2f}ms, {metrics.throughput_mb_per_second:.2f} MB/s"
                )
                return result
    ...
    def profile_serializer(
        self, serializer_cls, doc: DoclingDocument, **kwargs
    ) -> Dict[str, Any]:
        ...
        with self._profile_context(operation_name) as profiler:
            try:
                ...
                output_size = (
                    len(result.text) if hasattr(result, "text") else len(str(result))
                )
                ...
                result_dict = {
                    ...
                    "recommendations": self._generate_serializer_recommendations(
                        metrics
                    ),
                }

                logger.info(
                    f"Serializer profiling complete: {duration_ms:.2f}ms, {output_size} chars output"
                )
                return result_dict
    ...
    def memory_profile(self, func: Callable[..., T], *args, **kwargs) -> T:
        ...
        with self._memory_monitor() as memory_tracker:
            ...
            try:
                ...
                logger.info(
                    f"Memory profiling complete for {func.__name__}: {memory_tracker.peak_usage_mb:.2f} MB peak"
                )
                return result
    ...
    def _generate_reader_recommendations(
        ...
        if (
            ...
            and metrics.file_size_bytes
            > self.config.streaming_threshold_mb * 1024 * 1024
        ):
        ...
        if metrics.duration_ms > 10000:  # 10 seconds
            recommendations.append(
                "Long processing time - consider parallel processing or optimization"
            )

        # Throughput recommendations
        if metrics.throughput_mb_per_second < 1.0:
            recommendations.append(
                "Low throughput - check I/O performance and JSON parsing efficiency"
            )

        return recommendations

    def _generate_serializer_recommendations(
        self, metrics: PerformanceMetrics
    ) -> List[str]:
        ...
        if metrics.memory_usage_mb > self.config.memory_limit_mb * 0.8:
            recommendations.append(
                "High memory usage - consider streaming serialization"
            )
        ...
        if output_size > 10 * 1024 * 1024:  # 10MB
            recommendations.append(
                "Large output size - consider compact serialization options"
            )

        # Performance recommendations
        if metrics.duration_ms > 5000:  # 5 seconds
            recommendations.append(
                "Long serialization time - consider optimization or parallel processing"
            )
    ...
    def get_metrics_summary(self) -> Dict[str, Any]:
        ...
        with self._lock:
            ...
            successful_operations = len(
                [m for m in self.metrics_history if not m.error_occurred]
            )
            error_rate = (total_operations - successful_operations) / total_operations

            durations = [
                m.duration_ms for m in self.metrics_history if not m.error_occurred
            ]
            memory_usage = [
                m.memory_usage_mb for m in self.metrics_history if not m.error_occurred
            ]
            throughputs = [
                m.throughput_mb_per_second
                for m in self.metrics_history
                if m.throughput_mb_per_second > 0
            ]
