...
class BenchmarkResult:
    ...
    def performance_score(self) -> float:
        ...
        if self.avg_duration_ms > 0:
            stability_bonus = max(
                0, 10 - (self.std_dev_ms / self.avg_duration_ms) * 100
            )
        else:
            stability_bonus = 0
...
class BenchmarkSuite:
            ...
            report = {
                "benchmark_summary": {
                    ...
                    "successful_tests": len(
                        [r for r in self.results if r.success_rate == 1.0]
                    ),
                    "failed_tests": len(
                        [r for r in self.results if r.success_rate < 1.0]
                    ),
                    "avg_performance_score": (
                        sum(r.performance_score for r in self.results)
                        / len(self.results)
                        if self.results
                        else 0
                    ),
    ...
    def benchmark_readers(self) -> Dict[str, Any]:
        ...
        for complexity in BENCHMARK_COMPLEXITIES:
            test_file = self._create_test_docling_file(
                100 * 1024, complexity=complexity
            )
            ...
            try:
                ...
                scaling_results[f"doc_count_{count}"] = {
                    ...
                    "throughput_docs_per_sec": (
                        count / (duration / 1000) if duration > 0 else 0
                    ),
        ...
        for workers in worker_counts:
            ...
            try:
                    ...
                    with ThreadPoolExecutor(max_workers=workers) as executor:
                        ...
                        for file_path in test_files:
                            future = executor.submit(
                                self._load_document_worker, file_path
                            )
                ...
                concurrency_results[f"workers_{workers}"] = {
                    ...
                    "throughput_docs_per_sec": (
                        len(results) / (duration / 1000) if duration > 0 else 0
                    ),
                    "speedup": (
                        concurrency_results.get("workers_1", {}).get(
                            "duration_ms", duration
                        )
                        / duration
                        if "workers_1" in concurrency_results
                        else 1.0
                    ),
                    "efficiency": (
                        (
    ...
    def _benchmark_reader(
        ...
        if durations:
            ...
            avg_duration = min_duration = max_duration = std_dev = avg_memory = (
                peak_memory
            ) = 0
    ...
    def _benchmark_serializer(
        ...
        doc: DoclingDocument,
        test_name: str,
        serializer_params: Dict[str, Any],
        context: Optional[Dict[str, Any]] = None,
    ) -> BenchmarkResult:
        ...
        logger.debug(
            f"Benchmarking serializer {test_name} with {self.iterations} iterations"
        )
        ...
        if durations:
            ...
            peak_memory = max(memory_usages) if memory_usages else 0
            avg_output_size = (
                sum(output_sizes) / len(output_sizes) if output_sizes else 0
            )
        else:
            avg_duration = min_duration = max_duration = std_dev = avg_memory = (
                peak_memory
    ...
    def _create_test_docling_file(
        self, size_bytes: int, complexity: str = "medium"
    ) -> Path:
        ...
        target_text_items = max(
            1, (size_bytes - 1000) // estimated_item_size
        )  # Reserve 1KB for structure
        ...
        logger.debug(
            f"Created test file {file_path.name}: {actual_size} bytes (target: {size_bytes})"
        )
    ...
    def _generate_recommendations(self) -> List[str]:
        ...
        low_performers = [
            r for r in self.results if r.performance_score < avg_score * 0.7
        ]

        if low_performers:
            recommendations.append(
                f"Found {len(low_performers)} operations with below-average performance"
            )

        # Memory usage analysis
        high_memory_ops = [
            r for r in self.results if r.peak_memory_mb > 200
        ]  # 200MB threshold
        if high_memory_ops:
            recommendations.append(
                f"{len(high_memory_ops)} operations use >200MB memory - consider optimization"
            )
        ...
        if error_rate > 0.1:
            recommendations.append(
                f"High error rate ({error_rate:.2%}) - investigate failure causes"
            )

        # Throughput analysis
        avg_throughput = sum(r.throughput_mbps for r in self.results) / len(
            self.results
        )
        if avg_throughput < 1.0:
            recommendations.append(
                "Low average throughput <1 MB/s - consider I/O optimization"
            )
...
class ReaderBenchmark:
    ...
    def benchmark_document_complexity(
        self, reader_cls: Type[BaseReader]
    ) -> Dict[str, Any]:
        ...
        for complexity in BENCHMARK_COMPLEXITIES:
            ...
            result = self.suite._benchmark_reader(
                reader_cls, test_file, f"complexity_{complexity}"
            )
...
class SerializerBenchmark:
    ...
    def benchmark_serialization_options(
        self, serializer_cls, doc: DoclingDocument
    ) -> Dict[str, Any]:
        ...
        for config_name, params in configs.items():
            result = self.suite._benchmark_serializer(
                serializer_cls, doc, f"config_{config_name}", params
            )
